...
void Dynamic_array::insert(Dynamic_array &p, int i) {						//-
	...
	if (position.block_p->size + p.size >= BLOCK_SIZE && position.block_p->size > 0) {
	...
	if (position.i % BLOCK_SIZE > 0 && position.i % BLOCK_SIZE < (BLOCK_SIZE - 1)) {
		Block * new_block_p = new Block;
		new_block_p->size = p.size;
		new_block_p->next_p = NULL;

		for (int j = position.i, k = 0; k < p.size; k++, j++) {
			new_block_p->a[k] = position.block_p->a[j];
		}
		insert_blocks(position.block_p, new_block_p);
	...
	if (position.i % BLOCK_SIZE == (BLOCK_SIZE - 1)) {
		insert_blocks(position.block_p, copy_p);
	}
		// update total size								//-
	size += p.size;
...
void Dynamic_array::remove(int start, int end) {						//-
	...
	//case2: start == end, no remove required
	if (start == end) {
		return;
	}

	//find target and index target blocks
	Block_position position = find_block(start);
	Block_position ender = find_block(end);

	//case3: start and end are in the same block but end is also at the end of the block
	if (position.i == 0 && ender.i == 4) {
		remove_blocks(position.pre_block_p, position.block_p, ender.block_p);
	}

	//case4: start and end are in the same block but start is located in the middle of the block
	else if (position.block_p == ender.block_p) {
		if (position.i > 0 && ender.i == 4) {
			position.block_p->a[position.i] = position.block_p->a[BLOCK_SIZE - 1];
			position.block_p->size -= (BLOCK_SIZE - position.i - 1);
		}
	}

	//case5: start and end are in different blocks but start begins at the start of a block and end ends at the start of another block
	else if (position.i == 0 && ender.i == 0) {
		remove_blocks(position.pre_block_p, position.block_p, ender.pre_block_p);
	}

	//case6: end is in the middle of another block 
	else if (position.i == 0 && ender.i != 0 && ender.i < 4) {
		remove_blocks(position.pre_block_p, position.block_p, ender.pre_block_p);
		for (int i = ender.i; i < (ender.block_p->size); i++) {
			ender.block_p->a[i - ender.i] = ender.block_p->a[i];
		}
		ender.block_p->size -= ender.i;
	}

	//case7: start is in the middle of a block and end is at the start of another block
	else if (position.i > 0 && ender.i == 0) {
		remove_blocks(position.block_p, position.block_p->next_p, ender.pre_block_p);
		position.block_p->size = (position.i);
	}

	//case8: start is in the middle of a block and end is equal to the size of the block
	else if (position.i > 0 && end == size) {
		remove_blocks(position.block_p, position.block_p->next_p, ender.block_p);
		position.block_p->size = position.i;
	}

	//case9: start and end are in the middle of opposite blocks
	else if (position.i > 0 && ender.i <= 4) {
		if (position.block_p->next_p != ender.block_p) {
			remove_blocks(position.block_p, position.block_p->next_p, ender.pre_block_p);
		}
